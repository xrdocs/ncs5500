...
Leafs have VRF 10 with route-target RT:10:10 configured for inter-subnet routing. BGP neighborship is formed using evpn address-family between Leafs, DCI routers and Route-Reflectors. In order to receive routes from Leafs for VRF 10, we will have to configure VRF 10 on DCI routers as well.
...
Below output confirms the Host routes advertised by Leafs are learnt in VRF 10 on DCI router.  
