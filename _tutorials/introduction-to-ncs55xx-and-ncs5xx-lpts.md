...
| SDR             | Secure Domain Router. It provide a means of partitioning a router into multiple, independent routers                      |
...
When packet enters the In ingress pipeline, traps are generated based on forwarding lookup. The L3 packets undergo a FIB lookup and are classified as “for-us” packet if it has to be consumed by the router itself. This happens during first pass.  The 2 traps generated are RxTrapReceive (for L3 packets) and RxTrapUserDefine_RECEIVE_L2 (for L2 packets). By looking at the traps, the packets are recycled to ingress pipeline by skipping the egress pipeline lookups. The lookup key can contain values such as – source IP, destination IP, source L4 port, destination L4 port, interface, vrf and compression id. This TCAM lookup happens in PMF. The “for-us” control packets are then punted by encapsulating in the NPU header. In NPU, the “for-us” control packets undergo hardware TCAM lookup which will hit one of the protocol default TCAM entries. NPU header contains the control information such as – ingress port, destination node, trap code, trap qualifier, listener_tag, flow type, stats pointer etc. 
