...
1) A frame is received on the ingress interface of a NCS55xx or NCS5xx
2) On receiving the packet necessary layer 1 and 2 checks are performed the necessary layer 1 and layer 3 information is extracted from the packet and passes it to the forwarding ASIC.
3) Packet enters the ingress pipeline and traps are generated based on forwarding lookup. 
4) The L3 forwarding engine does a Forwarding Information Base (FIB) lookup and determines whether the packet is a locally destined for_us packet. This is the **first pass**. The two traps generated are **RxTrapReceive** (for L3 packets) and **RxTrapUserDefine_RECEIVE_L2** (for L2 packets). 
5) By looking at the traps, the packets are recycled to ingress pipeline by skipping the egress pipeline lookups. The lookup key can contain values such as: source IP, destination IP, source L4 port, destination L4 port, interface, vrf and compression id. 
6) This TCAM lookup happens in PMF. The “for-us” control packets are then punted by encapsulating in the NPU header. In NPU, the “for-us” control packets undergo hardware TCAM lookup which will hit one of the protocol default TCAM entries. NPU header contains the control information such as – ingress port, destination node, trap code, trap qualifier, listener_tag, flow type, stats pointer etc. 
7) LPTS entries are installed as 4 main groups i.e. IPv4, IPv6, CLNS and L2. In first pass the traps are different for L2 and L3 because the lookup tables are different which are generating the traps. After such classification, packets go into Software Path Process (SPP), that selects whether to send to L2/SPIO or NetIO process. The packets are received by SPP in IOS-XR by opening a raw socket to listen over the punt interface. SPP is a component that classifies packets in punt path to identify its client. Moreover, SPP does the parsing of npu header to extract the control information filled by npu. 
8) However, it retains the npu header. Based on this parsing results it decides the client. (In our case NetIO or L2/SPIO). In case, there is no proper decision, the packet is forwarded to NetIO as a default client. 
9) NetIO handles L3 Protocol Packets like ICMP, OSPF, BGP etc. It also processes ARP packets unlike ASR9K where ARP is via SPIO. Based on lookup result, the control packets will get policed and punted to LC CPU. In case of errors, the packets are also punted for diagnostics (even if npu decides it to drop) or for some action (to generate ICMP error) via traps from first pass itself. 10) When the ‘For Us’ packet is received by the LPTS decap node in NetIO, LPTS  does iFIB lookup and find the packet associated  protocol client and deliver the packet to the protocol stack. 
11) Sometimes, this lookup is skipped if listener tag is provided. The Streamlined Packet IO - SPIO is used by L2 processes CFM,STP,ARP etc. When a reaches from SPP to NetIO/SPIO, the control information is transferred. NetIO/SPIO strip off the npu header and process the actual packet.
12) The CPU runs the software processes that decapsulate the packet and deliver them to the correct stack.
...
Hope this content was useful and you understood the importance of LPTS and its implementation on the platform. To summarize LPTS consists of 3 key functions. Filtering of what can be punted and categorised as a flow.Decides where the flow needs to go to: is it LC CPU or RP CPU. Policing of the flows per NPU. Key message is that, LPTS takes control plane protection to new level and can be considered as a feature which is composed of a dynamic ACL's which are created as part of user configuration automatically. This is configuration driven and no user intervention is required. We covered how the exception and layer2 control packets are handled via traps in the hardware. We also looked how to change the default entries to accomodate user specific entries in LPTS. But whatever we saw is at the global level. This takes effect on the entire router. In the next tech note we will cover how we can fine tune the hardware policing specific to domains. Stay tuned till then !!!
