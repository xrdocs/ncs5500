...
Starting IOS XR release 7.10.1, we would support hardware offload for SR-PM probing functionality. This would significantly increase the probing frequency, that would enable us to achieve fast convergence (<50ms). We will revisit this feature when we delve into the Ethernet Teleprotection use case with stringent latency requirements.
